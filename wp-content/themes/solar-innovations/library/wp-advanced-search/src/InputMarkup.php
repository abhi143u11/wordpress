...
class InputMarkup extends StdObject {
    ...
    private function openWrapper() {
        ...
        if ($this->input->wrappersDisabled() === false) {
            $output .= '<div id="wpas-'.$id.'"  class="wpas-'
                .$id.' wpas-'.$this->input->getFieldType().'-field wpas-field '.$this->input->getClass().'">';
        }
        return $output;
    }
    ...
    private function listField($is_checkbox = true) {
        ...
        $active = (isset($_GET[$this->input->getId()])) ? ' active' : '';
        $extended = (strlen($active) > 0) ? 'true' : 'false';
		$output = '<ul class="accordion" data-accordion=""><li class="accordion-navigation' . $active . '"> <a href="#panel-'.$this->input->getId().'" aria-expanded="' . $extended . '">'.str_replace('_',' ',str_replace('meta_','',$this->input->getId())).'<i class="fa fa-chevron-right closed"></i><i class="fa fa-chevron-down open"></i></a><div id="panel-'.$this->input->getId().'" class="content' . $active . '"><div class="row">';
    ...
    private function input( $input_type = 'text' ) {
        ...
        $output = '<input type="'.$input_type.'" id="'.$this->input->getId().'" class="project-search wpas-'.$input_type.' '.$this->input->getClass().'" value="'.$value.'" name="'.$this->input->getInputName().'"'.$placeholder.' '.$this->attributesString().'><span class="text-center project-icon"><input type="image" alt="Search" src="'.get_bloginfo('template_directory').'/img/search-icon.png" /></span>';
        return $output;
    }
    ...
    private function reset() {
        ...
        return '<a href="' . get_bloginfo('url') . '/gallery" class="button tiny reset">Reset</a>';
    }

    private function buttonInput($type) {
        $values = $this->input->getValues();
        $value = reset($values);
        $output = '<input type="'.$type.'" class="button expand filter wpas-'.$type.' '.$this->input->getClass().'" value="Click to Filter" '.$this->attributesString().'><i class="fa fa-chevron-right"></i>';
    ...
    private function listOption($value, $label, $type = 'checkbox') {
        ...
        $name .= ($type == 'checkbox') ? '[]' : '';
        $output = '<div class="small-12 columns checks"><input type="'.$type.'" id="wpas-'.$id.'-'.$type.'-'.$ctr.'" class="checked wpas-'.$id.'-'.$type.' wpas-'.$type.'" name="'.$name.'" value="'.$value.'"';
        //$output = '<'.$element.' class="wpas-'.$id.'-'.$type.'-'.$ctr.'-container wpas-'.$id.'-'.$type.'-container wpas-'.$type.'-container">' . '<input type="'.$type.'" id="wpas-'.$id.'-'.$type.'-'.$ctr.'" class="wpas-'.$id.'-'.$type.' wpas-'.$type.'" name="'.$name.'" value="'.$value.'"';
        if (in_array($value, $this->input->getSelected(), true)) {
            $output .= ' checked="checked"';
        }
        $output .= '><label for="wpas-'.$id.'-'.$type.'-'.$ctr.'"> '.$label.'</label></div>';
        //$output .= '>';

...
//			),
		...
		$t[$i]->options[] = (object)array(
				'type'=>'subtitle',
				'label'=>__('Custom CSS','wlb')	
			);					

		$t[$i]->options[] =	(object)array(
				'id'		=> 'wp_admin_css',
				'label'		=> __('Valid css for the admin section','wlb'),
				'type'		=> 'textarea',
				'description'=> __('Write valid css to output in the wp-admin section.','wlb'),
	...
	function admin_head(){
		...
		$vars = array('wp_admin_css','dashboard_icon','header_logo','header_logo_link','header_logo_title','hide_blavatar','header_logo_width','header_bar_height','developer_logo','developer_name','developer_url','hide_update_nag','hide_update_download','hide_contextual_help','hide_screen_options','hide_favorite_actions','remove_wordpress_from_title','remove_footer_version');
		foreach($vars as $var){
...
<?php $this->favicon();?>
<style>
<?php if(trim($wp_admin_css)!=''):?>
<?php echo $wp_admin_css?>
<?php endif ?>

...
<?php			
	...
	function current_user_excluded_post_ids(){
		...
		$also_exclude = array();
		$extrafilter = '';
		//if(is_user_logged_in()){ //unnecesary check
		if(true){
			$wp_capabilities = $wpdb->prefix.'capabilities';
			if( property_exists($userdata,$wp_capabilities) && is_array($userdata->$wp_capabilities)&&count($userdata->$wp_capabilities)>0){
...
<?php
	...
	function public_dashboard_content(){
		global $wlb_plugin;
		echo stripslashes($wlb_plugin->get_option('panel_content'));
	}
	//------
	...
	function editor_dashboard_content(){
		global $wlb_plugin;
		echo stripslashes($wlb_plugin->get_option('editor_panel_content'));
	}	
	//------
	...
	function admin_dashboard_content(){
		global $wlb_plugin;
		echo stripslashes($wlb_plugin->get_option('administrator_panel_content'));
	}		
}
